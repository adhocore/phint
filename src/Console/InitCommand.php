...
class InitCommand extends Command
{
    ...
    protected function makeUsage()
    {
        ...
        $this->usage(
            ...
            . PHP_EOL . $colorizer->bold('  phint init') . ' <project> '
            . $colorizer->comment(
                '--php 7.0 --config /path/to/json --dev mockery/mockery --req adhocore/jwt --req adhocore/cli'
            )
        );
    }
    ...
    public function execute()
    {
        ...
        if ($this->using) {
            $io->write('Using ')->comment($this->using)->write(' to create project', true);
    ...
    protected function prepareProjectPath(): string
    {
        ...
        if (\is_dir($path)) {
            if (!$this->force) {
                throw new \InvalidArgumentException(\sprintf('Something with the name "%s" already exists!', \basename($path)));
            }
